...
export async function getDBDataPMZ(req: Request, res: Response) {
    const today = getTodayDate();
    try {
        const user = process.env.DENODO_USER;
        const password = process.env.DENODO_PASSWORD;

        const urlUint = 'http://100.101.1.13:9090/denodo-restfulws/zabbix/views/dv_dbitems_pmz_uint?$groupby=itemid&$select=itemid,first(host)%20as%20host,first(items_name)%20as%20items_name,first(itemid)%20as%20itemid,group_concat(value_max)%20as%20value,group_concat(clock)%20as%20clock,group_concat(date_br)%20as%20date_br&$filter=day=1&$format=json';

        const urlText = `http://100.101.1.13:9090/denodo-restfulws/zabbix/views/dv_dbitems_pmz_text?&$groupby=itemid&$select=itemid,first(host)%20as%20host,first(items_name)%20as%20items_name,first(itemid)%20as%20itemid,first(date_br)%20as%20date_Br,first(clock)%20as%20clock,first(value)%20as%20value&$format=json`;
        
        const urlServ = 'http://100.101.1.13:9090/denodo-restfulws/zabbix/views/dv_hosts_db_servidor_pmz?$format=json';
...
export async function getJiraPMZ(req: Request, res: Response) {
    ...
    try {
        ...
        const urlL =`http://100.101.1.13:9090/denodo-restfulws/jira/views/bv_jira_80d_fields_pmz?$format=json`;
        ...
        if (!responseList.status) {
         ...
         const urlC =`http://100.101.1.13:9090/denodo-restfulws/jira/views/dv_jira_contratos?$format=json&$filter=%22Cliente_10117%22like%27%25PMZ%25%27`;
...
export async function getLinksPMZPecas(req: Request, res: Response) {
    const month: string = req.body.month;
    console.log(month);
    try {
        const user = process.env.DENODO_USER;
        ...
        const url = `http://100.101.1.13:9090/denodo-restfulws/zabbix/views/dv_trends_healthcheck?$format=json&$groupby=itemid&$select=itemid,avg(value_min),avg(value_max),first(host),first(items_name)&$filter=date_time%3E%3D%27${startDate}%27%20AND%20date_time%3C%27${endDate}%27`;
        console.log(url);
        // Create Basic Auth header
        const authString = btoa(`${user}:${password}`);
